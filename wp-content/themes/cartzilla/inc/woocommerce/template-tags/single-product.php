...
/**
 * Output the product rating and "Add to  " button (if applicable)
 *
 * @see woocommerce_template_single_rating()
 * @see single-product/rating.php
 *
...
if ( ! function_exists( 'cartzilla_grocery_product_description_custom_fields' ) ) {
    function cartzilla_grocery_product_description_custom_fields() {
		global $product;
		?>
        <div class="product-custom-fiels mb-4">
			<div class='product-custom-fiels-details'>
			<?php 
				...
				foreach($fields as $key=>$field){					
					...
					if(is_array($field['value'])){
						...
						<?php							
						foreach($field['value'] as $keySub=>$subfield){
							?>
							<tr>
								<td><?php echo $subfield['velocidad']; ?></td>
								<td><?php echo $subfield['cantidad']; ?> </td>
							</tr>
							<?php
						} ?>
						</tbody>
			...
			<?php
			$stock = get_post_meta(get_the_ID(),'_stock_status');
			if(count($stock)>0)
			if($stock[0] == 'instock'){
			?>
			<hr>
			...
			<div id="app_stock" class="table-responsive">
				<table class="table table-bordered">
				<thead>
				  <tr>            
					<th>Bodega/Tienda</th>
			...
			<?php 
				//include get_template_directory() . '/assets/js/stock.js';
			?>			
			<script src="https://cdn.jsdelivr.net/npm/vue@2"></script>
			<script type='module'>
	...
	refresh:function(elements){
		for (let i in elements){
			let ele = elements[i]
			if(ele.onHand > 4){
				ele.onHand = 'En existencias'
			}
			if(ele.onHand <= 0){
				ele.onHand = 'Sin existencias'
			}
		}
	...
	const starCountRef = ref(database, 'stocks/'+sku);
    ...
    function cartzilla_toggle_single_product_hooks() {
        ...
        if ( 'style-v4' === $style ) { 
           ...
           add_action( 'woocommerce_single_product_summary',                       'cartzilla_grocery_loop_product_category',   5 );
			...
			?><hr><?php
            add_action( 'woocommerce_single_product_summary',                       'cartzilla_grocery_product_description_custom_fields',   30 );
			...
			add_action( 'cartzilla_product_meta',                       'cartzilla_grocery_product_description_custom_fields',   30 );
            ...
            add_action( 'cartzilla_info_tab_single_product_summary',                 'cartzilla_grocery_product_description_custom_fields',   63 );
            ...
            add_action( 'woocommerce_single_product_summary',                 		'cartzilla_grocery_product_description_custom_fields',   49 );
