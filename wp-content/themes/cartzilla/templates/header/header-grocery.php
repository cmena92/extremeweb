					...
					<?php if ( cartzilla_is_woocommerce_activated() ) : ?>
						<?php if( cartzilla_navbar_search_is_category_dropdown() ) { ?>
							<form class="input-group-overlay d-none d-lg-block" method="get" action="<?php echo esc_url( home_url( '/'  ) ); ?>" autocomplete="off">
							    ...
							    <input class="form-control prepended-form-control appended-form-control" type="text" placeholder="<?php echo esc_html_x( 'Buscar productos', 'front-end', 'cartzilla' ); ?>" name="s" value="<?php echo esc_attr( get_search_query() ); ?>" />
							    <div class="input-group-append-overlay">
									<?php $selected_cat = isset( $_GET['product_cat'] ) ? $_GET['product_cat'] : "0";
									$navbar_search_dropdown_text = apply_filters( 'cartzilla_navbar_search_category_dropdown_default_text', esc_html__( 'Todas las categorias', 'cartzilla' ) );
