...
<div class="modal fade" id="cpuModal" tabindex="-1" role="dialog" aria-labelledby="cpuModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
    <div class="modal-content">
      ...
      <div class="modal-header bg-info">
        <h5 class="modal-title text-light" id="cpuModalLabel">Procesadores</h5>
  ...
  <div class="row">
    <div class="col-sm-4 col-lm-4 col-md-4">
		...
		<label for="exampleFormControlSelect1">Busqueda avanzada</label>
							...
							<div class="row">
							  ...
							  <div class="col-8 col-sm-8">	
								<ul class="list-group list-group-flush">
									<li v-if="statusAcf(cpu.fields.cpu_frequency)" class="list-group-item">Frecuencia: {{cpu.fields.cpu_frequency}}</li>
									<li v-if="statusAcf(cpu.fields.cpu_socket)" class="list-group-item">Socket: {{cpu.fields.cpu_socket}}</li>
									<li v-if="statusAcf(cpu.fields.cpu_chipset)" class="list-group-item">Chipset: {{cpu.fields.cpu_chipset}}</li>
								</ul>	
							  ...
							  <div class="collapse" v-bind:id="cpu.collapseTarget">
								  <div class="">
									<ul class="list-group list-group-flush">
										<li v-if="statusAcf(cpu.fields.cpu_threads)" class="list-group-item">Hilos: {{cpu.fields.cpu_threads}}</li>
										<li v-if="statusAcf(cpu.fields.cpu_cache)" class="list-group-item">Cache: {{cpu.fields.cpu_cache}}</li>
										<li v-if="statusAcf(cpu.fields.cpu_energycost)" class="list-group-item">Consumo: {{cpu.fields.cpu_energycost}}</li>
										<li v-if="statusAcf(cpu.fields.cpu_cooling)" class="list-group-item">Inc. Enfriamiento: {{cpu.fields.cpu_cooling}}</li>
									</ul>
								  </div>
								</div>
								  ...
								  <button 
											...
											@click="setCpu(index)" 
											data-dismiss="modal"
											>Armar componente <i class="navbar-tool-icon czi-add"></i>
									</button>
									
								<button class="btn btn-outline-dark btn-sm float-right" type="button" data-toggle="collapse" v-bind:data-target="cpu.collapseTargetId" aria-expanded="false" aria-controls="collapseExample">
									Ver más detalles 
										<i v-if="" class="navbar-tool-icon czi-arrow-down"></i>  
